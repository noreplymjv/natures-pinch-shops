...
function updateStatusLine() {
  ...
  $("statusLine").textContent = `${state.allShops.length} shops · ${inCat} in this category · ${crmCount} with CRM`;
}
...
function applyFilters() {
  ...
  // When searching by name, search ALL shops (all categories + sections).
  // Status filter still applies. Without a query, keep category + section scope.
  const globalFind = Boolean(q);
